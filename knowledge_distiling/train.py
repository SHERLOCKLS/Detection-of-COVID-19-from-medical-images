...
if __name__ == '__main__':
    ...
    if "distill" in params.model_version:
        ...
        elif params.model_version == 'base_net_distill':
        ...
        """
        if params.teacher == "TS1":
            teacher_model = mynet.TS1()
            teacher_checkpoint = 'experiments/TS1/best.pth.tar'
            teacher_model = teacher_model.cuda() if params.cuda else teacher_model

        elif params.teacher == "TS2":
            teacher_model = mynet.TS1()
            teacher_checkpoint = 'experiments/TS1/best.pth.tar'
            teacher_model = teacher_model.cuda() if params.cuda else teacher_model

        elif params.teacher == "TS3":
            teacher_model = mynet.TS1()
            teacher_checkpoint = 'experiments/TS1/best.pth.tar'
            teacher_model = teacher_model.cuda() if params.cuda else teacher_model

        elif params.teacher == "TS4":
            teacher_model = mynet.TS1()
            teacher_checkpoint = 'experiments/TS1/best.pth.tar'
            teacher_model = teacher_model.cuda() if params.cuda else teacher_model
    ...
    # non-KD mode: regular training of the baseline CNN or mynet.base_net()
    else:
        ...
        elif params.model_version == "base_net":
            model = mynet.base_net().cuda() if params.cuda else mynet.base_net()
            optimizer = optim.SGD(model.parameters(), lr=params.learning_rate,
                                  momentum=0.9, weight_decay=5e-4)
            # fetch loss function and metrics
