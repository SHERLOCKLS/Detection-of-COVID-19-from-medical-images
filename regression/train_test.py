...
def main():
    ...
    train_data_list,val_data_list = train_test_split(train_,test_size = 0.2)#,stratify=train_["filename"]

    print(val_data_list)

    val_data_list.to_csv('val.csv',index=False)
    #4.5.4 load datase4
    ...
    if eval_only:
        ...
        for i in tqdm(range(500)):
            valid_loss = evaluate(val_dataloader,model,criterion,i*0.002)
    ...
    for epoch in range(start_epoch,config.epochs):
        ...
        for iter,(input,target) in enumerate(train_dataloader):
            ...
            print('%0.4f %5.1f %6.1f        | %0.3f  %0.3f  %0.3f         | %0.3f  %0.3f  %0.3f         |         %s         | %s' % (\
                         ...
                         avg, avg, avg,
                         avg, avg, avg,str(best_precision_save),
