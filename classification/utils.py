...
def save_checkpoint(state, is_best,loss_min,fold):
    ...
    if loss_min:
        shutil.copyfile(filename, config.best_models + config.model_name+ os.sep +str(fold)  + os.sep + 'lowest_loss.pth.tar')
...
def predictor(output, target, topk=(1,),T):
    with torch.no_grad():
        maxk = max(topk)
        batch_size = target.size(0)
        _, pred = output.topk(maxk, 1, True, True)
        pred = pred.t()
        correct = pred.eq(target.view(1, -1).expand_as(pred))
    	C2= confusion_matrix(correct, pred, labels=[0, 1])
    	tn,fp,fn,tp = C2.ravel()
    	Sensitivity.append(tp/(tp+fn))
    	Specificity.append(1-(fp/(fp+tn)))
    	Precisions.append(tp/(tp+fp))
    	Recalls.append(tp/(tp+fn))
        if max(Sensitivity)> T:
            return 1
        else:
            return 0

def counter(input,flow_pass):
    if input==0:
    	flow_pass = 0
        global_cnt = 0
    else:
